...
export function normalizeJSONValue(value: unknown): ChatJSONValue {
  if (
    value === null ||
    typeof value === "string" ||
    typeof value === "number" ||
...
export function decodeThreadSummary(value: unknown): ChatThreadSummary {
...
export function decodeThreadsResponse(value: unknown): ChatThreadSummary[] {
...
export function decodeCreateThreadResponse(value: unknown): { thread_id: string } {
  if (!isObject(value) || !asString(value.thread_id)) {
...
export function decodeEnterChatResponse(value: unknown): ChatEnterResponse {
  if (
    !isObject(value) ||
...
export function decodeCommittedTurn(value: unknown): ChatCommittedTurn {
  ...
  const toolCalls = rawToolCalls
    .filter((toolCall): toolCall is Record<string, unknown> => isObject(toolCall))
    .map((toolCall) => ({
...
export function decodeModelListResponse(value: unknown): ChatModelOption[] {
