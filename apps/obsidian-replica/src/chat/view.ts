...
import type { ChatThreadSessionState, ChatTranscriptItem, ChatViewState } from "../types.js";
...
export class SheafChatView extends ItemView {
  ...
  private threadListEl: HTMLElement | null = null;
  private conversationEl: HTMLElement | null = null;
  private conversationTitleEl: HTMLElement | null = null;
  private conversationSubtitleEl: HTMLElement | null = null;
  ...
  private renderThreadList(state: ChatViewState): void {
    const { contentEl } = this;
  ...
  private renderConversation(state: ChatViewState): void {
  ...
  private buildConversationShell(container: HTMLElement): void {
  ...
  private itemSignature(item: ChatTranscriptItem): string {
    switch (item.kind) {
      case "tool_call":
  ...
  private updateTranscriptItem(element: HTMLElement, item: ChatTranscriptItem): void {
  ...
  private renderTranscriptItem(item: ChatTranscriptItem): HTMLElement {
