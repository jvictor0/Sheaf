...
test("transcript ordering is committed then pending then streaming", () => {
    ...
    assert.deepEqual(items.map((item) => item.kind), ["committed", "pending", "streaming"]);
});
...
test("dropQueueArtifacts only clears matching queue state", () => {
    ...
    assert.deepEqual(session.pendingSends.map((pending) => pending.clientMessageID), ["pending-2"]);
...
test("tool summary keeps only file-safe label", () => {
    assert.equal(summarizeToolCall({
        id: "tool-1",
        name: "write_note",
...
test("assistant transcript includes tool call summary before the turn", () => {
    ...
    assert.deepEqual(session.transcriptItems.map((item) => item.kind), ["tool_call", "committed"]);
});
