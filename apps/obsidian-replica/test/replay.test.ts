...
import { ReplicaReplayEngine, type ReplicaRemoteReader, type ReplicaVaultAdapter } from "../src/replay.js";
import type { ReplicaLogRecord, ReplicaVaultState } from "../src/types.js";

function baseState(): ReplicaVaultState {
  return {
    version: 1,
    vaultName: "test",
    nextLsn: 2,
...
class MemoryAdapter implements ReplicaVaultAdapter {
...
test("patch replay applies unified diff before falling back", async () => {
  ...
  let fetchCalls = 0;
  const remote: ReplicaRemoteReader = {
    async fetchRawFile() {
      fetchCalls += 1;
